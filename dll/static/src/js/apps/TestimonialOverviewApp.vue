<template>
  <div>
    ...
    <div
      ...
      :key="index"
      class="content-box"
      :class="'content-box--' + review.type"
    >
      <div class="row">
        <div class="col-sm-3">
          <div class="content-box__type">
            {{ review.type_verbose }} ({{ review.status_display }})
          </div>
          <div class="content-box__title">
            {{ review.name }}
          </div>
          <div class="content-box__author">
            <span class="fas fa-user" /> {{ review.author }}
          </div>
          <div
        ...
        <div
          v-if="mode === 'reviewer' && review.status !== 4"
          class="col"
        >
          <ul
            v-if="review.status == 0 || review.status === 1"
            class="content-box__actions"
          >
      ...
      <div
        v-if="mode === 'user' && review.status === 4"
        class="row mt-3"
      >
        <div class="col">
          <textarea
            v-model="review.testimonial_comment"
            class="form-control"
          />
        </div>
      ...
      <div
        v-if="(review.status === 0 || review.status === 1) && review.change && mode === 'reviewer'"
        class="row mt-3"
      >
        <div class="col-12">
          <textarea
          ...
          />
          <div
            v-if="review.commentError"
            class="alert alert-danger mt-2"
            v-text="review.commentError"
          />
        </div>
        <div class="col-12 mt-2">
          <button
            class="btn btn-primary"
            @click="requestChange(review)"
          >
            Änderungen anfragen
          </button>
        </div>
