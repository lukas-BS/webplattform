...
urlpatterns = [
    ...
    path(
        "login/",
        auth_views.LoginView.as_view(template_name="dll/user/login.html"),
        name="login",
    ),
    path("profil/", login_required(ProfileViewIndex.as_view()), name="profile"),
    path(
        "profil/passwort-aendern/",
        login_required(ProfileViewChangePassword.as_view()),
    ...
    path(
        "profil/loeschen/",
        login_required(ProfileViewDelete.as_view()),
        name="account_delete",
    ),
    path(
        "profil/loeschen/erfolgreich/",
        ProfileViewDeleteSuccess.as_view(),
    ...
    path(
        ...
        auth_views.PasswordResetDoneView.as_view(
            template_name="dll/user/password_reset_done.html"
        ),
